...
class SlackBlockManager:
    ...
    def update_with_blocks(client, channel: str, ts: str, blocks: List[dict], 
        ...
        try:
            ...
            # text가 None이거나 빈 경우 기본 메시지 설정
            if not text or not text.strip():
                text = "메시지가 표시되지 않으면 Slack 앱을 업데이트해주세요."
                logger.warning(f"Empty text provided, using default: {text}")
            ...
            logger.error(f"Channel: {channel}, TS: {ts}, Text length: {len(text) if text else 0}")
            return False
