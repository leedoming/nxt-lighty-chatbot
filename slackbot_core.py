...
def conversation(say, query: str, thread_ts: Optional[str] = None,
    ...
    try:
        ...
        try:
            initial_text = Config.BOT_CURSOR if Config.BOT_CURSOR and Config.BOT_CURSOR.strip() else "처리 중..."
        ...
        if thread_ts or is_dm:  # 스레드가 있거나 디엠일 때 이전 대화 조회
            ...
            try:
                # 이전 메시지 확인 중 상태 업데이트
                update_text = MSG_PREVIOUS if MSG_PREVIOUS and MSG_PREVIOUS.strip() else "이전 대화 내용 확인 중..."
                ...
                if is_dm:
                    ...
                    thread_history = SlackManager.get_dm_history(channel, client_msg_id)
                else:
                    # 스레드의 경우 기존 방식 사용
        ...
        try:
            update_text = MSG_RESPONSE if MSG_RESPONSE and MSG_RESPONSE.strip() else "응답 생성 중..."
            SlackManager.update_message(say, channel, thread_ts, latest_ts, update_text)
        ...
        if Config.USE_AGENT and bedrock_agent_runtime is not None:
            ...
            try:
                ...
                # 블록 형식 메시지로 업데이트
                logger.info("Updating message with blocks...")
                ...
                if success:
                    ...
                    # raw_message를 최종 message로 설정 (DynamoDB 저장용)
                    message = raw_message
                else:
                    # 블록 업데이트 실패 시 일반 텍스트로 폴백
                    logger.warning("Failed to update with blocks, falling back to plain text")
                    message = response_parser.get_plain_text_fallback(parsed_data)
        ...
        # 메시지 데이터베이스에 저장 (디엠의 경우 채널 ID 사용)
        ...
        try:
            if 'latest_ts' in locals():
                ...
                error_message = "죄송합니다. 처리 중 예기치 못한 오류가 발생했습니다. 잠시 후 다시 시도해주세요."
...
class SlackManager:
    ...
    @staticmethod
    @staticmethod
    def get_dm_history(channel: str, client_msg_id: str, limit: int = 10) -> List[str]:
        """Retrieve conversation history from a DM channel (최근 4개 대화만)"""
        ...
        try:
            ...
            # 최근 3개 대화만 선택
            recent_pairs = conversation_pairs[-3:] if len(conversation_pairs) > 4 else conversation_pairs
...
class BedrockManager:
    ...
    def invoke_agent(prompt: str) -> str:
        ...
        try:
            ...
            # Call Bedrock Agent with timeout
            import time
            ...
            for event in response.get("completion", []):
                try:
                    ...
                    if chunk and "bytes" in chunk:
                        ...
                        # Log every 10 chunks or for first/last chunk
            ...
            if "ThrottlingException" in str(e):
                ...
                return "요청 형식에 문제가 있습니다. 관리자에게 문의해주세요."
    ...
    try:
        ...
        try:
            if 'latest_ts' in locals():
                logger.info("Attempting to send error message to user")
                error_message = "죄송합니다. 처리 중 예기치 못한 오류가 발생했습니다. 잠시 후 다시 시도해주세요."
                SlackManager.update_message(say, channel, thread_ts, latest_ts, error_message)
                logger.info("Error message sent to user successfully")
        except Exception as inner_e:
            logger.error(f"Failed to send error message: {inner_e}", exc_info=True)
